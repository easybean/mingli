...
- `stages`：**人生阶段标签（重要）**，数组，从下表 7 个 id 里选；引擎按用户当前年龄只浮**本阶段**的题，错阶段的题直接不进池。**不写 = 各阶段通用**（仅限"焦虑失眠 / 攀比内耗 / 自我怀疑"这类全人群都有的题；其余一律要标 stages）。一张题可属多个阶段，如 `["earlyCareer","establish","midlife"]`。
- `gender`：`"all"`(默认，不写即此) / `"female"` / `"male"`。**绝大多数题 all**；只有备孕、生育、婚嫁催促、更年期这类才标性别。标了性别的题，性别不符的用户基本不会看到。
- `choices`：**正好 3 个**，`style` 必须 `bold`(直接推进) / `steady`(稳着来) / `repair`(先修边界关系) 各一。

**人生阶段 7 段（stages 取值）：**

| id | 阶段 | 年龄 | 典型处境（同一主题在不同阶段是不同的题） |
|---|---|---|---|
| `teen` | 少年求学 | 0–17 | 中学、升学压力、同伴、家庭、自我认同 |
| `college` | 大学青年 | 18–23 | 学业/保研、室友同学、社团、初恋、实习求职、生活费 |
| `earlyCareer` | 初入社会 | 24–29 | 职场新人、租房、恋爱婚恋、第一份工作、原生家庭拉扯 |
| `establish` | 立业成家 | 30–39 | 事业爬坡、婚姻、育儿、买房、父母渐老 |
| `midlife` | 中年 | 40–49 | 事业天花板/转型、青春期子女、赡养、健康预警、婚姻倦怠 |
| `senior` | 中老年 | 50–64 | 退休准备、子女独立婚嫁、健康、空巢、再就业 |
| `elder` | 老年 | 65+ | 退休生活、健康、与子女关系、含饴弄孙、身后安排 |

> 注意："career 事业"主题在 `college` 阶段写的是**学业/升学/实习**（学生没有同事上下级），到 `establish` 才是带团队、扛 KPI。每个阶段都按这个阶段真实会遇到的事来写。
...
本次任务：为【人生阶段=大学青年 college】×【主题=人际 network】写【5】张今日题(event)，
每张挂不同的命理触发条件（覆盖：左右扶命格 / 比肩劫财偏重 / 巨门 / 仆役坐煞 等），
情境必须是这个阶段（大学生）真实会遇到的事（室友/同学/社团，没有同事上下级）。

硬性要求：
0. 每张题必须带 `stages:["college"]`（按本次阶段）；分性别才写 gender，否则省略。

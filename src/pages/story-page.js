...
export const renderStoryPage = (state) => {
  ...
  const model = createWorkStoryViewModel({ definition, profile: profileForStoryDefinition(definition, state.astrolabeData), session: state.workStorySession });
  if (!model.ready || !model.node) return `<section class="page"><div class="empty-state"><p>${escapeHtml(state.ui.error || `正在生成这一次${definition?.themeLabel || '人生岔路'}推演。`)}</p><button class="button button-primary" data-page="birth">重新填写出生信息</button></div></section>`;
  const { node, feedback } = model;
  return `
    <section class="page story-page">
      <header class="story-header">
        <div><p class="page-kicker">${escapeHtml([model.themeLabel, model.careerStageLabel, `${model.progress.current} / ${model.progress.total}`].filter(Boolean).join(' · '))}</p><h1>${escapeHtml(model.displayTitle)}</h1></div>
